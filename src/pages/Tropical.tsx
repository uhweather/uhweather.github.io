...
import { floaterUrl, TROPICAL } from '../lib/sources'
...
function Outlook() {
  const [id, setId] = useState(TROPICAL.outlooks[1].id)
  const shown = TROPICAL.outlooks.find((o) => o.id === id) ?? TROPICAL.outlooks[0]

  const text = useQuery({
    queryKey: ['two', shown.basin],
    queryFn: () => nws.latestProduct('TWO', shown.basin === 'Central Pacific' ? 'CP' : 'EP'),
    staleTime: 30 * 60_000,
    refetchInterval: 30 * 60_000,
    refetchIntervalInBackground: true,
  })
  ...
  return (
    <Card
      ...
      subtitle="Areas being watched for development, and the odds given to each."
      actions={
        <SegmentedControl
          label="Outlook"
          value={id}
          onChange={setId}
          options={TROPICAL.outlooks.map((o) => ({
            id: o.id,
            label: `${o.basin.split(' ')[0]} ${o.range}`,
            title: `${o.basin}, ${o.range} outlook`,
          }))}
        />
      }
    >
      <div className="grid gap-5 lg:grid-cols-2">
        <img
          key={shown.url}
          src={shown.url}
          alt={`${shown.basin} ${shown.range} tropical weather outlook`}
          className="w-full rounded border border-line bg-white object-contain"
        />
        {text.data?.productText ? (
          <div className="min-w-0">
            <p className="text-xs text-faint">
              Issued {relativeAge(text.data.issuanceTime)} · {shown.basin}
            </p>
            <pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap font-mono text-xs leading-relaxed text-muted">
              {text.data.productText.trim()}
            </pre>
          </div>
        ) : (
